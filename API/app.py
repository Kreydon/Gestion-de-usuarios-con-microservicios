...
log_db_config = {
    ...
    'user': 'root',
    'password': '123456',
    'database': 'log_database',  # Update with your actual log database name
}
...
def obtener_usuario_por_id(id_usuario):
    try:
        connection = get_user_db_connection()
        cursor = connection.cursor(dictionary=True)
        consulta = 'SELECT * FROM usuarios WHERE subrogada = %s'
...
# Endpoint to search users by filters
...
def buscar_usuarios():
    try:
        ...
        if tipo_documento:
            consulta += ' AND tipoDocumento = %s'
        if genero:
            consulta += ' AND genero = %s'

        cursor.execute(consulta, (tipo_documento, genero))
...
def agregar_usuario():
    try:
        ...
        cursor.execute("SELECT * FROM usuarios WHERE noDocumento = %s", (datos['noDocumento'],))
        existing_user = cursor.fetchone()

        if existing_user:
            return jsonify({'error': 'Ya existe un usuario con el mismo ID'}), 400

        consulta = "INSERT INTO usuarios (tipoDocumento, noDocumento, firstName, secondName, apellidos, fechaNacimiento, genero, correoElectronico, celular, fechaActualizacion, estado, foto) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
...
def update_user(user_id):
    try:
        ...
        query = "UPDATE usuarios SET tipoDocumento=%s, noDocumento=%s, firstName=%s, secondName=%s, apellidos=%s, fechaNacimiento=%s, genero=%s, correoElectronico=%s, celular=%s, fechaActualizacion=%s, estado=%s, foto=%s WHERE subrogada=%s"
...
def delete_user(user_id):
    try:
        ...
        query = "DELETE FROM usuarios WHERE subrogada=%s"
